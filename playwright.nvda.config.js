 ...
 *
 * Separate from the project's main Playwright config on purpose: these
 * tests drive a real, headed Firefox with a silent portable NVDA attached
 * (Guidepup), so they cannot run in parallel, cannot run headless, and take
 * the keyboard while they run. Run them with `npm run test:sr` on demand.
 *
