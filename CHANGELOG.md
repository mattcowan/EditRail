...
* Extensions can store their own data on a pinned tool: `window.toolrail.setPinData(slot, namespace, value)` and `getPinData()`, plain JSON up to 2 KB per namespace, kept in the same entry so it is removed with the pin, saved with sets and carried in set files. The rail does not read it; a tool registered under the pin gives it meaning. Every change to a pin's entry fires the `toolrail:pin-meta-changed` window event.
