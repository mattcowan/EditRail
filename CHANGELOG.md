...
* Edit a pinned tool's name, description and icon: an Edit button beside each tool under "Pinned tools" in Toolbar settings opens an inline form. Empty fields use the block's own. The icon is up to three characters, or a Dashicon: a Browse icons button beside the field opens a searchable list of every Dashicon WordPress ships, and a typed name that is not one of them is refused. The description is the tool's tooltip and its accessible description. Stored per user under one preference key (`toolrail-pin-meta`) and removed with the tool when it is unpinned. Saved sets record the labels of their pins (`toolrail-set-meta`), carry them in set files as an optional `meta` object, and apply them when the set is loaded. `window.toolrail.getPinMeta()` and `setPinMeta()` read and write a pin's own entry.
* Extensions can store their own data on a pinned tool: `window.toolrail.setPinData(slot, namespace, value)` and `getPinData()`, plain JSON up to 2,048 characters per namespace, kept in the same entry so it is removed with the pin, saved with sets and carried in set files. The rail does not read it; a tool registered under the pin gives it meaning. Every change to a pin's entry fires the `toolrail:pin-meta-changed` window event.
