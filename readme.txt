...
* While a tool is armed, the editor's own "+" button between blocks is hidden, so your click goes to the armed tool. A checkbox under "Inserting" in Toolbar settings turns this off.
...
* A Help panel ("?" next to the gear) explains inserting, pinning, moving the toolbar, the keyboard model, saved sets, and what a highlighted tool means. It opens once on your first visit, after the editor's own welcome guide is closed; after that, only when you ask. You can hide the "?" from the toolbar — the panel stays available from Toolbar settings.
...
= 0.1.22 =
* The Section tool is now the Group block, pinned by default at the head of the pinned blocks. Unpin it, move it, or put it in a saved set like any other pin. It inserts what Section did: a group with one paragraph, ready to type in. An account that already had the toolbar gets Group added once, ahead of its own pins; a toolbar you emptied stays empty. "Restore default tools" now restores Group too. A plugin that nests a tool under `section` keeps working: the name now points at the pinned Group.
* Fix: with a tool armed, the editor's own "+" button between blocks (and at the foot of a Cover) took the click and opened the block picker instead of inserting the armed block. The "+" is now hidden while a tool is armed. A new checkbox under "Inserting" in Toolbar settings turns this off.
* Fix: an armed click in the gap between two blocks put the new block at the end of the document. It now goes between the two blocks, inside the same parent.
* Fix: on a fresh account, the Help panel opened under the editor's "Welcome to the editor" guide, and the click that closed the guide closed the Help panel too, so you never read it. The first-run Help panel now waits until the guide is closed.
