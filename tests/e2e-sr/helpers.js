 ...
 *  - Build editor state through wp.data with Playwright — nothing to hear
 *    there.
 ...
 * install the window.__qa helpers (see qa-helpers.js).
 */
...
/** Describe document.activeElement (tag, className, id, aria-label, data-tool, text). */
async function describeFocus(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      tool: (a.dataset && a.dataset.tool) || null,
...
/** Delete the scratch draft the editor auto-created. Returns its ID, or false. */
async function deleteCurrentPost(page) {
  return page
    .evaluate(async () => (window.__qa ? window.__qa.deleteCurrentPost() : false))
    .catch(() => false);
}

/** True when document.activeElement is inside the element matched by selector. */
