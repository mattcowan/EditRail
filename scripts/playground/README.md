...
"Editor Tool Rail" contains none of the ligature pairs, so the headline block turns on `ss01` (the alternate "a" in "Rail") and the tagline carries the ligatures: "**Th**e ar**ch**ite**ct**'s toolbar. Pick a tool **fi**r**st**, then click the canvas; every block **st**ays a core block." A caption that claims a feature the letters cannot show is worse than none, so change the copy and the feature list together.
